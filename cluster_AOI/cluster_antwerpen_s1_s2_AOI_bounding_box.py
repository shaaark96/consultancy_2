...
# Funktion zur Erstellung von AOIs als Bounding Boxes
def erstelle_bounding_box_aoi(df, stadt=None, city_map=None, user=None, n_clusters=3):
    if stadt:
    ...
    for i in range(n_clusters):
        ...
        plt.gca().add_patch(
            plt.Rectangle((min_x, min_y), max_x - min_x, max_y - min_y, fill=False, color='blue', linestyle='--'))

    plt.title(f'Bounding Box AOIs für Stadt={stadt}, CityMap={city_map}, Benutzer={user}')
...
erstelle_bounding_box_aoi(df, stadt='Antwerpen', city_map='Antwerpen_S1', user='p17', n_clusters=3)
erstelle_bounding_box_aoi(df, stadt='Antwerpen', city_map='Antwerpen_S2', user='p4', n_clusters=3)
