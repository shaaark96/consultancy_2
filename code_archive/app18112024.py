    ...
    html.Div([
        ...
        html.Div([
            html.Div([
                ...
                html.Div([
                    html.H3('Cluster Analysis for Color Data'),
                    dcc.Dropdown(
                ...
                ], className='cluster_analysis_container')
...
# 3.2 - Update Output Section and Plot Area based on active button:
@app.callback(
...
def update_user_dropdowns(selected_city):
    if selected_city:
        try:
            ...
            # Log or handle unexpected data issues
            return [[], []]
...
def update_dropdown_classname(current_theme):
    return f'dropdown {current_theme}_theme_dropdown'
...
def update_grey_plot(selected_users, selected_city, selected_aoi_type, n_clusters, current_theme):
    if selected_city:
        # Define a color map for users
        unique_users = df['user'].dropna().unique()
        colors = px.colors.qualitative.Plotly
...
def update_color_plot(selected_user, selected_city, selected_aoi_type, n_clusters):
    ...
    image_path_color, width, height = get_image_path_color(selected_city)
    if not image_path_color or not width or not height:
        print("Error: Bildpfad oder Dimensionen nicht gefunden.")
        return go.Figure()
    ...
    if len(coords) < n_clusters:
        print("Error: Not enough data points for the specified number of clusters.")
        return go.Figure()
    ...
    fig.update_layout(
        title=f'AOI-Visualisierung für Color ({selected_aoi_type}, {n_clusters} Cluster)',
